module RegexHelper
  ...
  # the following regex needs at least one character which should be a
  # valid unicode letter or number; allows spaces, underscore, hyphen
  # shouldn't start with a underscore, hyphen, or space
  # shouldn't end with a space
  UNICODE_CHARACTER_NUMBER_HYPHEN_UNDERSCORE = Regexp.new('\A[\p{L}\p{N}]+(?:[\p{L}\p{N}_\- ]*[\p{L}\p{N}_-])?\Z')
  ...
  WHATSAPP_BSUID_PATTERN = '[A-Za-z0-9][A-Za-z0-9._-]*'
