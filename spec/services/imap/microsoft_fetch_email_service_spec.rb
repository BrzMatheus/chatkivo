...
RSpec.describe Imap::MicrosoftFetchEmailService do
  ...
  def expect_peek_fetch_for_new_email(since_date)
    expect_peek_fetch_commands(since_date)
    expect_fetch_summary_log
  end

  def expect_peek_fetch_commands(since_date)
    expect(imap).to have_received(:search).with(['SINCE', since_date])
  ...
  def expect_fetch_summary_log
    expect(logger).to have_received(:info).with(
      "[IMAP::FETCH_EMAIL_SERVICE] Inbox #{microsoft_channel.inbox.id}: fetching mails from #{microsoft_channel.email}, found 1."
    )
  end
    ...
    context 'when new emails are available in the mailbox' do
      it 'fetches the emails and returns the emails that are not present in the db' do
        travel_to '26.10.2020 10:00'.to_datetime do
          ...
          expect_peek_fetch_for_new_email('25-Oct-2020')
        end
      end
    end
    ...
    context 'when the interval is passed during an IMAP Sync' do
      it 'fetches the emails based on the interval specified in the job' do
        travel_to '26.10.2020 10:00'.to_datetime do
          ...
          expect(result.length).to eq 1
          expect(result[0].message_id).to eq email_object.message_id
          expect_peek_fetch_for_new_email('18-Oct-2020')
        end
      end
    end
  end
